...
import { Upload, Music, Download, RefreshCw, Volume2, AlertCircle, X, Check } from 'lucide-react';
...
export default function AudioToMp3() {
  ...
  const selectFile = (selectedFile: File) => {
    ...
    const hasAudioExtension = /\.(m4a|mp3|wav|aac|flac|ogg|opus|wma|mp4)$/i.test(selectedFile.name);
    
    if (!isAudioType && !hasAudioExtension) {
      setErrorMsg('Please select a valid audio file (e.g. M4A, WAV, AAC, FLAC, MP3).');
      setStatus('error');
      setFile(null);
      return;
    }
  ...
  const startConversion = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Could not decode the audio file. Please check if the file format is supported by your browser.');
      } finally {
            ...
            {!file ? (
              ...
              >
                <input 
                  ...
                  accept="audio/*, .m4a, .wav, .aac, .flac, .ogg, .mp3, .opus, .wma" 
                  style={{ display: 'none' }} 
                />
                <div className="upload-content">
                  ...
                  <h3 style={{ fontSize: '15px' }}>Drag & drop your audio file here</h3>
                  <p style={{ fontSize: '12px' }}>Supports M4A, WAV, AAC, FLAC, OGG, MP3, etc.</p>
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '12.5px', fontWeight: 500, color: 'var(--text-secondary)' }}>
                        {status === 'reading' && 'Reading audio file...'}
                        {status === 'decoding' && 'Decoding audio tracks...'}
